...
export default function ClientMediaGrid({
    ...
    const [originalLoading, setOriginalLoading] = useState<string | null>(null);
    ...
    /** Photo ki ORIGINAL quality dikhao (thumbnail nahi) — album select karne ke liye */
    const handleViewOriginal = async (e: React.MouseEvent, item: MediaItem) => {
        e.stopPropagation();
        setOriginalLoading(item.mediaId);
        try {
            const res = await fetch(`/api/clients/media/${item.mediaId}/original`);
            const result = await res.json();
            if (result.success) {
                setPreview({ url: result.data.url, type: "photo" });
            } else {
                alert(result.error || "Original photo load nahi ho payi");
            }
        } finally {
            setOriginalLoading(null);
        }
    };
                        ...
                        >
                            ...
                            {item.fileType === "photo" && (
                                <button
                                    ...
                                    onClick={(e) => handleViewOriginal(e, item)}
                                    disabled={originalLoading === item.mediaId}
                                    title="Original quality me dekhein"
                                >
                                    {originalLoading === item.mediaId ? (
                                        <span className="loader" />
                                    ) : (
                                        "🔍 Original"
                                    )}
